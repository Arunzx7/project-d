...
const addProduct = async (req, res) => {
  upload(req, res, function (err) {
    ...
    const { product_title, description, price, category, size, stock} = req.body; // brand
  ...
  try {
    ...
    const userData = req.session.user;
      if (userData) {
          ...
          return res.render('user/productDetails', { product});
      }else{
          // const cartLength = userCart ? userCart.items.length : 0;
          return res.render('user/productDetails', { product});
      }
...
// const productDetails = async (req, res) => {
//   try {
//     const products = await Products.find(); // Assuming you fetch all products
//     const userData = req.session.user;

//     if (userData) {
   
//       return res.render('user/productDetail', { products, userData });
//     } else {
//       return res.render('user/productDetail', { products, userData: null });
//     }
//   } catch (error) {
//     console.error('Error occurred while rendering product details page:', error);
//     res.status(500).send('Internal Server Error');
//   }
// };
